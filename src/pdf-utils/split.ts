 ...
 */
export async function splitPDFToPDFs(options: SplitPDFToPDFsOptions): Promise<SplitPDFToPDFsResponse> {
  try {
    ...
    if (splitMethod === 'extract' && extractRange) {
      ...
      // Split every N pages
      if (pages <= 0) {
        return { success: false, error: 'Pages per split must be greater than 0' };
      }
      ...
      for (let startPage = 0; startPage < totalPages; startPage += pages) {
        ...
        pdfBlobs.push(pdfBlob);
        filenames.push(`${baseName}_part_${Math.floor(startPage / pages) + 1}.pdf`);
      }
      ...
      for (let i = 0; i < rangeList.length; i++) {
        ...
        pdfBlobs.push(pdfBlob);
        filenames.push(`${baseName}_pages_${range.replace(/[^\d-]/g, '_')}.pdf`);
      }
    } else {
      return { 
        success: false, 
 ...
 */
export async function splitPDFToImages(options: SplitPDFToImagesOptions): Promise<SplitPDFToImagesResponse> {
  try {
    ...
    if (format !== 'jpeg' && format !== 'png') {
      return { success: false, error: 'Invalid format. Supported formats: jpeg, png' };
    }
