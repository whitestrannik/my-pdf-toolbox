...
describe.skip("CombinePDFsView", () => {
  it("renders the combine PDFs interface", () => {
    ...
    expect(
      screen.getByText(/Merge multiple PDF files into a single document/),
    ).toBeInTheDocument();

    // Check for dropzone
    expect(screen.getByText("Drop PDF files here")).toBeInTheDocument();
    expect(
      screen.getByText("or click to browse (max 20MB each)"),
    ).toBeInTheDocument();
  });
  ...
  it("shows file size limit in description", () => {
    ...
    expect(
      screen.getByText(/Maximum file size: 20MB per file/),
    ).toBeInTheDocument();
  });
